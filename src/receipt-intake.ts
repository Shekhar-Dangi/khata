...
//
// ONE implementation, called by BOTH the upload route and the re-parse route. the design
// gives the reason directly — "what must not happen is business logic accumulating inside a
// script and then being re-implemented behind a route; that is how two paths drift into
// disagreeing about dedupe". Here the thing they would drift on is what an artifact's
// parse_status means, which is the state a re-run reads to decide what to do.
...
export type PdfIntake = {
  /** 'parsed' | 'unsupported' | 'failed' — mirrors what was written to the artifact row. */
...
export async function intakePdf(
  ...
  // Unreachable while PARSERS_AVAILABLE is all false. Left explicit rather than omitted so
  // Phase B is a change in one obvious place instead of a new branch someone has to invent.
  await setParseStatus(client, artifactId, "failed", {
    sourceType: template,
    error: `parser for ${template} is registered but not wired up`,
  });
  return {
    status: "failed",
    template,
    pages,
    tables,
    message: `parser for ${template} is registered but not wired up`,
  };
}
